  ...
  try {
    ...
    if (req.file) {
      ...
      if (user.profilePicture) {
        const existingFilePath = user.profilePicture.replace(
          `${process.env.BASE_URL}/media/profiles/`,
          ""
        );
...
exports.updatePatientTreatmentInfo = async (req, res) => {
  ...
  try {
    ...
    if (patient.group !== "patient") {
      return res
        .status(403)
        .send({ message: "Can only update treatment info for patients" });
    }
    ...
    res
      .status(200)
      .send({ message: "Treatment information updated successfully", patient });
  } catch (error) {
    console.error(error);
...
exports.forgotPassword = async (req, res) => {
  ...
  try {
    ...
    resetUrl = `${process.env.FRONTEND_URL}:3000/reset-password?token=${resetToken}`;
...
exports.updateSettings = async (req, res) => {
  try {
    ...
    if (videoUploadAlert) {
      ...
      reminderFrequency =
        reminderFrequency !== undefined ? reminderFrequency : 60;
    }
    ...
    res
      .status(500)
      .send({ message: "Error updating settings", error: error.message });
  }
};
...
exports.getSettings = async (req, res) => {
  ...
  try {
    const user = await User.findById(userId).select(
      "videoUploadAlert reminderTime reminderFrequency"
    );
    if (!user) {
