...
exports.uploadVideoForDates = async (req, res) => {
  ...
  if (!patient || !patient.diagnosisDate) {
    return res
      .status(404)
      .json({ message: "Patient not found or diagnosis date is missing." });
  }
  ...
  const endDate =
    currentUTCDate.getFullYear() === +year &&
    currentUTCDate.getMonth() + 1 === +month
      ? currentUTCDate
      : new Date(Date.UTC(year, month, 0)); // Last day of specified month in UTC
...
exports.getVideosByPatientId = async (req, res) => {
  try {
    ...
    if (videos.length === 0) {
      return res
        .status(404)
        .json({ message: "No videos found for the specified patient." });
    }
